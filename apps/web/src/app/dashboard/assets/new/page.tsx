    ...
    <div style={{ padding: "1rem", maxWidth: "700px", margin: "0 auto" }}>
      ...
      <form onSubmit={handleSubmit}>
        <div style={{ marginBottom: "1rem" }}>
          ...
          <select value={form.location_id} onChange={e=>setForm({...form, location_id: e.target.value})} required style={{ width: "100%", padding: "0.75rem" }}>
        ...
        <div style={{ marginBottom: "1rem" }}>
          <label style={{ display: "block", marginBottom: "0.5rem", fontWeight: 500 }}>Tipo de activo</label>
          <select value={form.asset_type_id || ""} onChange={e=>setForm({...form, asset_type_id: e.target.value || undefined})} style={{ width: "100%", padding: "0.75rem" }}>
            <option value="">Seleccionar tipo</option>
            {assetTypes.map(t=>(<option key={t.id} value={t.id}>{t.label}</option>))}
          </select>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem", marginBottom: "1rem" }}>
          <div><label style={{ display: "block", marginBottom: "0.25rem" }}>Marca</label><input value={form.brand} onChange={e=>setForm({...form, brand: e.target.value})} style={{ width: "100%", padding: "0.75rem" }} /></div>
          <div><label style={{ display: "block", marginBottom: "0.25rem" }}>Modelo</label><input value={form.model} onChange={e=>setForm({...form, model: e.target.value})} style={{ width: "100%", padding: "0.75rem" }} /></div>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem", marginBottom: "1rem" }}>
          <div><label style={{ display: "block", marginBottom: "0.25rem" }}>Número de serie</label><input value={form.serial_number} onChange={e=>setForm({...form, serial_number: e.target.value})} style={{ width: "100%", padding: "0.75rem" }} /></div>
          <div><label style={{ display: "block", marginBottom: "0.25rem" }}>Código interno</label><input value={form.internal_code} onChange={e=>setForm({...form, internal_code: e.target.value})} style={{ width: "100%", padding: "0.75rem" }} /></div>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem", marginBottom: "1rem" }}>
          <div><label style={{ display: "block", marginBottom: "0.25rem" }}>QR Code</label><input value={form.qr_code} onChange={e=>setForm({...form, qr_code: e.target.value})} style={{ width: "100%", padding: "0.75rem" }} /></div>
          <div><label style={{ display: "block", marginBottom: "0.25rem" }}>Estado</label>
            <select value={form.status || "ACTIVE"} onChange={e=>setForm({...form, status: e.target.value as "ACTIVE"|"INACTIVE"|"RETIRED"})} style={{ width: "100%", padding: "0.75rem" }}>
        ...
        <div style={{ marginBottom: "1rem" }}>
          ...
          <textarea value={form.description} onChange={e=>setForm({...form, description: e.target.value})} rows={3} style={{ width: "100%", padding: "0.75rem" }} />
        </div>
