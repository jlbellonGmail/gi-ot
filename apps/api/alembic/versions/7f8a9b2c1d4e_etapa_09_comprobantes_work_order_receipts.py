...
def upgrade() -> None:
    # La aplicación ya utilizaba SyncOperation mediante create_all en tests,
    # pero ninguna revisión histórica creaba la tabla. Se incorpora aquí
    # porque la propia revisión 09 la referencia y un replay limpio no podía
    # alcanzar HEAD ni en SQLite ni en PostgreSQL.
    op.create_table(
        "sync_operations",
    ...
    op.create_table(
        ...
        sa.Column(
            "sent_to_email", sa.Boolean(), nullable=False, server_default=sa.false()
        ),
        sa.Column(
            "sent_to_whatsapp", sa.Boolean(), nullable=False, server_default=sa.false()
        ),
    ...
    op.create_index(
        "ix_work_order_receipts_tenant_id",
        "work_order_receipts",
        ["tenant_id"],
    )
